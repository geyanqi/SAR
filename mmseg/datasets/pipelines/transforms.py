            ...
            if key == 'gt_sup_feat':
                ...
                if self.keep_ratio:
                    ...
                    gt_seg = mmcv.imrescale(
                        results[key], (int(results['img'].shape[0]/8+ 0.5), int(results['img'].shape[1]/8+0.5)), interpolation=interp)
                else:
                    gt_seg = mmcv.imrescale(
                        results[key], (int(results['img'].shape[0]/8+ 0.5), int(results['img'].shape[1]/8) + 0.5), interpolation=interp)
                ...
                if self.keep_ratio:
                    gt_seg = mmcv.imrescale(
                        results[key], results['scale'], interpolation=interp)
                else:
...
class Pad(object):
    ...
    def _pad_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            if key == 'gt_sup_feat':
                padded_sup_feat = mmcv.impad(
                    results[key],    
                    shape=(60,60),
                    pad_val=0)
                results[key] = padded_sup_feat
                pad_mask = np.zeros((results['gt_sup_feat'].shape[:2]))
                pad_mask = mmcv.impad(
                    pad_mask,    
                    shape=(int(results['pad_shape'][0]/8 + 0.5), int(results['pad_shape'][1]/8)),
                    pad_val=1)
                key = 'gt_sup_feat_pad_mask'
                results[key] = pad_mask
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            if key == 'gt_semantic_seg':
                ...
                crop_feat_bbox = (int(crop_bbox[0]/8 + 0.5), int(crop_bbox[1]/8 +0.5), \
                    int(crop_bbox[2]/8 + 0.5), int(crop_bbox[3]/8+0.5))
                results['gt_sup_feat'] = self.crop(results['gt_sup_feat'], crop_feat_bbox)
        return results
